...
class BasicReproductionStrategy(ReproductionStrategy):
    ...
    def reproduce(self, parents: Tuple[Individual, Individual], fitness_function: FitnessFunc) -> Individual:
        ...
        if slice_index == 0:
            ...
            new_array[:slice_index] = parents[0].as_array().reshape(new_array.size)[:slice_index]
            new_array[slice_index:] = parents[1].as_array().reshape(new_array.size)[slice_index:]

        return Individual(new_array.reshape(parents[0].as_array().shape), fitness_function)
